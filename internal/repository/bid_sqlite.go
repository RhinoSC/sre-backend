...
func (r *BidSqlite) FindAll() (bids []internal.Bid, err error) {
	rows, err := r.db.Query("SELECT b.`id`, b.`bidname`, b.`goal`, b.`current_amount`, b.`description`, b.`type`, b.`create_new_options`, b.`status`, b.`run_id`, bo.`id`, bo.`name` AS bid_option_name, bo.`current_amount`, bo.`bid_id` AS bid_option_current_amount FROM `bids` AS b LEFT JOIN bid_options AS bo ON b.`id` = bo.`bid_id`;")
	if err != nil {
	...
	for rows.Next() {
		...
		err = rows.Scan(&bid.ID, &bid.Bidname, &bid.Goal, &bid.CurrentAmount, &bid.Description, &bid.Type, &bid.CreateNewOptions, &bid.Status, &bid.RunID, &bidOption.ID, &bidOption.Name, &bidOption.CurrentAmount, &bidOption.BidID)
		if err != nil {
			logger.Log.Error(err.Error())
			return
		}
...
func (r *BidSqlite) FindById(id string) (bid internal.Bid, err error) {
	row := r.db.QueryRow("SELECT b.`id`, b.`bidname`, b.`goal`, b.`current_amount`, b.`description`, b.`type`, b.`create_new_options`, b.`status`, b.`run_id` FROM `bids` AS b WHERE b.`id` = ?", id)

	err = row.Scan(&bid.ID, &bid.Bidname, &bid.Goal, &bid.CurrentAmount, &bid.Description, &bid.Type, &bid.CreateNewOptions, &bid.Status, &bid.RunID)
	if err != nil {
...
func (r *BidSqlite) Save(bid *internal.Bid) (err error) {
	...
	_, err = tx.Exec("INSERT INTO `bids` (`id`, `bidname`, goal, current_amount, description, type, create_new_options, status, run_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", bid.ID, bid.Bidname, bid.Goal, bid.CurrentAmount, bid.Description, bid.Type, bid.CreateNewOptions, bid.Status, bid.RunID)
	if err != nil {
		tx.Rollback()
...
func (r *BidSqlite) Update(bid *internal.Bid) (err error) {
	...
	_, err = tx.Exec("UPDATE `bids` SET `bidname` = ?, `goal` = ?, `current_amount` = ?, `description` = ?, `type` = ?, `create_new_options` = ?, `status` = ?, `run_id` = ? WHERE `id` = ?;", bid.Bidname, bid.Goal, bid.CurrentAmount, bid.Description, bid.Type, bid.CreateNewOptions, bid.Status, bid.RunID, bid.ID)
	if err != nil {
		tx.Rollback()
		var sqliteErr sqlite3.Error
		if errors.As(err, &sqliteErr) {
